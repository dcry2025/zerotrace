...
describe('notesService', () => {
	...
	describe('checkNoteStatus', () => {
		...
		it('should handle API errors', async () => {
			vi.mocked(notesApi.notesApi.checkNoteStatus).mockRejectedValue(new Error('Note not found'));

			await expect(notesService.checkNoteStatus('invalid-link')).rejects.toThrow('Note not found');
		});
	});
});
