...
import {
	...
	type ReadNoteResponseDto,
	type NoteStatusResponseDto
...
const notesService = {
	...
	createNote: async (data: CreateNoteInput): Promise<CreateNoteResult> => {
		try {
			...
			const encrypted = await encryptText(data.content, noteKey, {
			...
			if (!publicKey) {
				throw new Error(
					'Master public key not configured. Run: node scripts/generate-master-key.js'
				);
			}
	...
	readNote: async (
		...
		try {
			...
			if (error instanceof DecryptionError) {
				throw new Error(
					'Failed to decrypt note: Invalid encryption key, corrupted data, or tampered content.'
				);
			}
