...
describe('RSA encryption module', () => {
	...
	describe('encryptKeyForAdmin', () => {
		...
		it('should throw error for invalid noteKey', async () => {
			...
			await expect(encryptKeyForAdmin(null as any, testPublicKey)).rejects.toThrow(
				RSAEncryptionError
			);
		});
	...
	describe('decryptKeyForAdmin', () => {
		...
		it('should throw error for invalid encrypted key', async () => {
			...
			await expect(decryptKeyForAdmin('invalid-base64!@#', testPrivateKey)).rejects.toThrow(
				RSADecryptionError
			);
		});
		...
		it('should throw error for invalid private key', async () => {
			...
			await expect(decryptKeyForAdmin(encryptedKey, 'not-a-key')).rejects.toThrow(
				RSADecryptionError
			);
		});
		...
		it('should throw error when decrypting with wrong private key', async () => {
			...
			const wrongPrivateKeyBase64 = btoa(
				String.fromCharCode(...new Uint8Array(wrongPrivateKeyExport))
			);
			const wrongPrivateKey = `-----BEGIN PRIVATE KEY-----\n${wrongPrivateKeyBase64}\n-----END PRIVATE KEY-----`;

			await expect(decryptKeyForAdmin(encryptedKey, wrongPrivateKey)).rejects.toThrow(
				RSADecryptionError
			);
		});
	});
