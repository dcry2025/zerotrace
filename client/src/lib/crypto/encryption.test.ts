...
describe('encryption module', () => {
  ...
  describe('encryptText and decryptText', () => {
    ...
    it('should throw EncryptionError for invalid plaintext', async () => {
      ...
      await expect(encryptText(undefined as any, noteKey, metadata)).rejects.toThrow(EncryptionError);
    ...
    it('should throw EncryptionError for invalid key', async () => {
      await expect(encryptText(plaintext, 'invalid-key', metadata)).rejects.toThrow(EncryptionError);
    ...
    it('should throw EncryptionError for invalid metadata', async () => {
      ...
      await expect(encryptText(plaintext, noteKey, { uniqueLink: '' } as any)).rejects.toThrow(EncryptionError);
    });
    ...
    it('should throw DecryptionError for invalid encrypted data', async () => {
      ...
      await expect(decryptText({ ciphertext: 'test' } as any, noteKey)).rejects.toThrow(DecryptionError);
    });
  });
