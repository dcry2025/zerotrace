 ...
 */
export async function decryptText(encrypted: EncryptedData, noteKey: string): Promise<string> {
	try {
		// Validate inputs
		...
		if (error instanceof DOMException) {
			if (error.name === 'OperationError') {
				throw new DecryptionError(
					'Decryption failed: Invalid key, corrupted data, or tampered metadata',
					error
				);
			}
			if (error.name === 'InvalidAccessError') {
...
function base64urlDecode(str: string): Uint8Array {
	try {
		...
		throw new Error(
			`Invalid base64url string: ${error instanceof Error ? error.message : 'decode failed'}`
		);
	}
}
