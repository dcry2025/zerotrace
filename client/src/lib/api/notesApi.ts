...
const notesApi = {
	...
	async createNote(data: CreateNoteDto): Promise<CreateNoteResponseDto> {
		try {
			const response = await ky
				.post(`${API_BASE_URL}${API_PREFIX}/notes`, {
					json: data,
					timeout: 30000
				})
	...
	async readNote(uniqueLink: string, data?: ReadNoteDto): Promise<ReadNoteResponseDto> {
		try {
			const response = await ky
				.post(`${API_BASE_URL}${API_PREFIX}/notes/${uniqueLink}/read`, {
					json: data || {},
					timeout: 30000
				})
	...
	async destroyNote(uniqueLink: string): Promise<{ success: boolean; message: string }> {
		try {
			const response = await ky
				.delete(`${API_BASE_URL}${API_PREFIX}/notes/${uniqueLink}`, {
					timeout: 30000
				})
	...
	async destroyNoteByDeleteLink(
		...
		try {
			const response = await ky
				.delete(`${API_BASE_URL}${API_PREFIX}/notes/delete/${deleteLink}`, {
					timeout: 30000
				})
				.json<{ success: boolean; message: string }>();
