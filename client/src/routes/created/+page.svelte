<script lang="ts">
	...
	let botInfo = $state<{ username: string | null; link: string | null; ready: boolean } | null>(
		null
	);
...
<div
	class="flex min-h-screen max-w-full flex-col overflow-x-hidden bg-gradient-to-br from-slate-900 via-green-900 to-blue-900"
>
	<!-- Header -->
	<Header subtitle={$t('created.success.subtitle')} />

	<!-- Main Content -->
	<main class="mx-auto w-full max-w-4xl flex-1 overflow-x-hidden px-4 pt-6 lg:px-8 lg:pt-8">
		<!-- Success Message Component -->
		<SuccessMessage title={$t('created.success.title')} subtitle={$t('created.success.subtitle')} />
		...
		<DestroyNoteButton {uniqueLink} onDestroy={handleDestroyNote} {destroying} {destroyed} />

		<!-- Telegram Bot Card Component -->
		{#if showTelegramCard}
			<TelegramBotCard {botInfo} {uniqueLink} {loadingBotInfo} onOpenBot={handleOpenTelegramBot} />
		{/if}
	</main>
