...
describe('NotesService', () => {
  ...
  describe('getAllNotes', () => {
    it('should return paginated notes with default parameters', async () => {
      ...
      mockNoteModel.findAndCountAll.mockResolvedValue(mockNotes);
    ...
    it('should enforce maximum limit of 500', async () => {
      ...
      const result = await service.getAllNotes(1000);
    ...
    it('should enforce minimum limit of 1', async () => {
      ...
      const result = await service.getAllNotes(0);
    ...
    it('should enforce minimum offset of 0', async () => {
      ...
      const result = await service.getAllNotes(10, -5);
    ...
    it('should correctly calculate hasMore flag', async () => {
      ...
      const result = await service.getAllNotes(20, 0);
  ...
  describe('getAllNotesForAdmin', () => {
    it('should return notes with encrypted content and keys', async () => {
      const mockNotes = {
        count: 10,
        rows: [
          {
      ...
      const result = await service.getAllNotesForAdmin();

      expect(result.notes[0]).toHaveProperty('encryptedContent');
      expect(result.notes[0]).toHaveProperty('encryptedKeyForAdmin');
    });

    it('should enforce limit and offset bounds', async () => {
      const mockNotes = {
        count: 10,
        rows: [],
      };
      ...
      const result = await service.getAllNotesForAdmin(1000, -10);
  ...
  describe('getNoteById', () => {
    it('should return note by ID', async () => {
      ...
      expect(result.content).toBe('Test content');
      expect(mockNoteModel.findByPk).toHaveBeenCalledWith(1);
    });

    it('should throw NotFoundException if note not found', async () => {
      mockNoteModel.findByPk.mockResolvedValue(null);

      await expect(service.getNoteById(999)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.getNoteById(999)).rejects.toThrow(
        NOTES_ERRORS.NOTE_NOT_FOUND,
